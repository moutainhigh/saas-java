 ...
 *
 */
public class ZmkSendSmsCode extends YpSmsMgrTemplate{
	public static ZmkSendSmsCode getInstance() {
		return HotSwap.getInstance().getSingleton(ZmkSendSmsCode.class);
	}
	...
	public String sendSms(Map<String, String> map, List<String> phones, String apikey) throws Exception {
		...
		if(super.isChinaPhone(phone)) {
			text = replaceTemplate(ThirdPartyServerCfgMgr.getProp().getZmkCodeTemplate(), map);
		}else if(super.isGatPhone(phone)){//港澳台短信
			text = replaceTemplate(ThirdPartyServerCfgMgr.getProp().getZmkGatCodeTemplate(), map);
		}else if(phone.startsWith(symbol)){//国际短信
			text = replaceTemplate(ThirdPartyServerCfgMgr.getProp().getZmkIntlCodeTemplate(), map);
		}else{//国内短信
			text = replaceTemplate(ThirdPartyServerCfgMgr.getProp().getZmkCodeTemplate(), map);
		}
