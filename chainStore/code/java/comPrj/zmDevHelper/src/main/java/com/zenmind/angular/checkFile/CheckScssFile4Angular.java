...
/**
 * 用于检查scss文件名 与 导入的文件名是否一致【区分大小写】
 * @author Administrator
 *
 */
public class CheckScssFile4Angular {

	private final String suffix = ".scss";
	private final String checkPath="F:/honkon/Saas/chainStore/code/h5/zmtWeb/src/zmWeb";
	
	public static void main(String[] args) throws Exception {
		new CheckScssFile4Angular().doCheck();
	}
	...
	public void doCheck() throws Exception {
		...
		//过滤ts类型的文件
		List<File> filterTsFiles = filterFilesByType(allFiles, ".ts");
		//检查不符合文件名称的导入语句
		checkFileName(filterTsFiles, fileNames);
	...
	private boolean isGetLine(String line){
		line = line.trim();
		if(line.indexOf("styleUrls") > -1 && line.indexOf(".scss") > -1){
			return true;
		}
		return false;
	}
	...
	private String getLineContent(String line){
		...
		line = line != null ? line.replaceAll("\"", "").replaceAll("'", "").replaceAll(".scss", ""):"";
		return line.trim();
	}
