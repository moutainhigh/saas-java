...
public class ChainPackageProject {
	@Id
	private long id;
	...
	public static ChainPackageProject newInstance(long chainId) {
		ChainPackageProject chainPackageProject = new ChainPackageProject();
		chainPackageProject.id = chainId;
		chainPackageProject.chainId = chainId;
		
		PackageProjectType type = PackageProjectType.newInstance();
		String typeId = SysInitTypeEnum.UnClassify.getId();
		type.setId(PackageProjectType.generateId(chainId, Long.valueOf(typeId)));
		type.setName(SysInitTypeEnum.UnClassify.getName());
		chainPackageProject.packageProjectTypeMap.put(type.getId(), type);

		long curTime = System.currentTimeMillis();
		chainPackageProject.createdTime = curTime;
		chainPackageProject.lastUpdateTime = curTime;
