...
public class MainLog {
	...
	public static void info(LogModule logModule, String id, String info) {
		...
		//info信息  不用发送
//		LogHelper.getInstance().info(logModule, id, info);
	}
	...
	public static void error(LogModule logModule, String id, String reason) {
		...
		LogHelper.getInstance().error(logModule, id, reason, null);
	}

	public static void error(LogModule logModule, String id, String reason,
			Throwable throwable) {
		logger.error("{}|{}|{}\n{}", logModule.getName(), id, reason, throwable);
		LogHelper.getInstance().error(logModule, id, reason, getStackTrace(throwable));
	}
