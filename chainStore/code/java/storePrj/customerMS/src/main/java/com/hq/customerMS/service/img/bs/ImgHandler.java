package com.hq.customerMS.service.img.bs;
...
import com.hq.customerMS.service.img.apiData.FileUploadApiForm;
import com.hq.customerMS.service.img.apiData.FilesUploadApiForm;
import com.hq.customerMS.service.img.apiData.ImgResp;
...
public class ImgHandler {
	...
	public ReqResult<ImgResp> saveImg(FileUploadApiForm apiForm) {
		...
		try {
			...
			MainLog.error(LogModule.Image, "ImgHandler[saveImg]", "", e);
		}
		return result;
	}
	...
	public ReqResult<ImgResp> saveImgs(FilesUploadApiForm apiForm) {
		...
		try {
			...
			result.setStatus(RespStatus.INTERNAL_SERVER_ERROR);
			result.setTips("服务暂不可用，请稍后尝试。");
			MainLog.error(LogModule.Image, "ImgHandler[saveImgs]", "", e);
		}
		return result;
	}
}
