 ...
 * 1, 改成单例<br>
 * 2, 使config可以动态切换（存在线程安全性问题）
 * @author: wujunwei 
 * @version: v1.0  
 * @since: JDK 1.8
 */
public class ZmNormalWXPay {

	public static ZmNormalWXPay getInstance(){
		return HotSwap.getInstance().getSingleton(ZmNormalWXPay.class);
	}
    ...
    private ZmNormalWXPayRequest wxPayRequest;
...
//            this.signType = SignType.HMACSHA256;
        ...
        ZmNormalWXPayRequest.getInstance().init(config);
        this.wxPayRequest = ZmNormalWXPayRequest.getInstance();
    }
     ...
     * @param configP
     */
    public void setConfig(WXPayConfig configP) {
    	this.config = configP;
    	ZmNormalWXPayRequest.getInstance().setConfig(configP);
    }
