...
public class OrderQueryForm {
	...
	public ReqMap toReqMap() {
		...
		reqMap.add("storeId", storeId).add("numberOrName", numberOrName).add("orderType", orderType)
				...
				.add("maxPayTime", maxPayTime).add("minPayTime", minPayTime).add("maxTime", maxTime)
				.add("minTime", minTime).add("leaguerId", leaguerId).add("cuserId", cuserId).add("buserId", buserId);
		return reqMap;
	}
