...
import com.hq.storeManagerRestClient.common.validate.info.ValidateInfo;
...
public class RestProxySTImpl implements IntfRestProxy{
	...
	private HttpHeaders buildHeaders(){
		...
		ValidateInfo info = ValidateThreadLocal.getInstance().getValidateInfo();
		if (info != null) {
...
//		ResponseEntity<RestResp> resp = (ResponseEntity<RestResp>) getTemplate().exchange(uri, HttpMethod.PUT, entity, RestResp.class);
...
//		ParameterizedTypeReference<RestResp<T>> pt = new ParameterizedTypeReference<RestResp<T>>() {}; 
//		ResponseEntity<RestResp> resp = (ResponseEntity<RestResp>) getTemplate().exchange(uri, HttpMethod.GET, entity, RestResp.class);
		ResponseEntity<RestResp> resp = send(uri, HttpMethod.GET, entity);
		if(resp.getStatusCode().is2xxSuccessful()){
			RestResp restResp = resp.getBody();
			if(restResp.getCode()==200){
