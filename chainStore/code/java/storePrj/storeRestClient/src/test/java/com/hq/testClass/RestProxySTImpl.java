...
public class RestProxySTImpl implements IntfRestProxy{
	...
	private HttpHeaders buildHeaders(){
		...
		ValidateInfo info = AccessTokenMgr.getInstance().getValidateInfo();
		if(info != null){
...
//		ResponseEntity<RestResp> resp = (ResponseEntity<RestResp>) getTemplate().exchange(uri, HttpMethod.PUT, entity, RestResp.class);
	...
	private ResponseEntity<RestResp> send(String uri,HttpMethod method, HttpEntity<String> entity){
		...
		try {
			ThreadRecorder.logReq("RestProxySTImpl");
			PressRecorder.logReq("RestProxySTImpl");
		...
		ThreadRecorder.logFinish("RestProxySTImpl");
		return respEntity;
	}
